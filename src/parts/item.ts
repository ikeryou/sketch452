...
export class Item extends MyObject3D {
  ...
  constructor(_id:number, v0:Vector2, v1:Vector2, v2:Vector2) {
    ...
    for(let i = 0; i < num; i++) {
      ...
      const col = new Color(0x000000).offsetHSL(Util.random(0, 1), (Util.hit(2) ? Util.random(0, 1) : 0), Util.random(0, 1))
      ...
      const m = new Mesh(
        ...
        new ShaderMaterial({
          ...
          side:DoubleSide,
          uniforms:{
            tDiffuse:{value:TexLoader.instance.get(Conf.instance.PATH_IMG + 'sample2.png')},
            rot:{value:0},
            fix:{value:1},
            rate:{value: 0},
  ...
  protected _update():void {
    ...
    const range = Util.map(d, 1, 0, 0, area) * 0.001
